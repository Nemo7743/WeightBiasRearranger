...
def parse_isa_to_bin(file_path, output_path):
    ...
    for base_row in instruction_start_rows:
        ...
        for signal in final_signal_order:
            ...
            # 數據清洗與解析
            is_nan = pd.isna(raw_val) or val_str == '' or val_str.upper() == 'X' or val_str.lower() == 'nan'

            if is_nan:
                parsed_int_val = 0
            
            # TBO 處理 (保持原有邏輯：將 Hex 字串轉為 Bin)
        ...
        if instruction_bin_str:
            # 這裡直接儲存連續的二進制字串 (例如: 11001010...)
            # 如果需要每4位一個空格 (例如: 1100 1010)，請取消下方兩行的註解並替換掉下一行
            # bin_chunks = [instruction_bin_str[i:i+4] for i in range(0, len(instruction_bin_str), 4)]
            # bin_outputs.append(" ".join(bin_chunks))
...
# 執行轉換
#input_excel = r'VLIW_Excel/Unit1_1_DownSamplingL.xlsx'
#input_excel = r'VLIW_Excel/Unit1_2_DownSamplingR.xlsx'
#input_excel = r'VLIW_Excel/Unit2_to_4_ShuffleUnit.xlsx'

#input_excel = r'VLIW_Excel/Unit5_1_DownSamplingL.xlsx'
#input_excel = r'VLIW_Excel/Unit5_2_DownSamplingR.xlsx'
#input_excel = r'VLIW_Excel/Unit6_to_12_ShuffleUnit.xlsx'

#input_excel = r'VLIW_Excel/Unit13_1_DownSamplingL.xlsx'
#input_excel = r'VLIW_Excel/Unit13_2_DownSamplingR.xlsx'
#input_excel = r'VLIW_Excel/Unit14_to_16_ShuffleUnit.xlsx'

input_excel = r'VLIW_Excel/Unit17_ConvLast_ClobalAveragePool_FC.xlsx'


#output_txt = r'VLIW_txt/Unit1_1_DownSamplingL.txt' # 這個檔案現在會包含二進制內容
#output_txt = r'VLIW_txt/Unit1_2_DownSamplingR.txt'
#output_txt = r'VLIW_txt/Unit2_to_4_ShuffleUnit.txt'

#output_txt = r'VLIW_txt/Unit5_1_DownSamplingL.txt'
#output_txt = r'VLIW_txt/Unit5_2_DownSamplingR.txt'
#output_txt = r'VLIW_txt/Unit6_to_12_ShuffleUnit.txt'

#output_txt = r'VLIW_txt/Unit13_1_DownSamplingL.txt'
#output_txt = r'VLIW_txt/Unit13_2_DownSamplingR.txt'
#output_txt = r'VLIW_txt/Unit14_to_16_ShuffleUnit.txt'

output_txt = r'VLIW_txt/Unit17_ConvLast_ClobalAveragePool_FC.txt'
...
if isinstance(result, list):
    ...
    if len(result) > 0:
        print(f"指令位元寬度 (Bits): {len(result[0].replace(' ', ''))}") # 顯示總 Bit 數
else:
    print(result)