...
def process_dw_weights_column_major(weight_file, bias_file, output_folder):
    ...
    for i in range(num_filters):
        ...
        # 檢查 3x3 權重是否完整
        if len(weights) < 9:
            print(f"警告：第 {i} 組權重不足 9 個，跳過。")
            continue

        # 1. 處理 Bias: 拆分高 4 位與低 4 位 (bias[0:4], bias[4:8])
        ...
        with open(output_path, 'w') as f:
            ...
            f.write(f"{bias_high} {bias_low} 0000 0000\n")
            ...
            # Column 0: 索引 0, 3, 6
            f.write(f"{weights[0]} {weights[3]} {weights[6]} 0000\n")
            
            # Column 1: 索引 1, 4, 7
            f.write(f"{weights[1]} {weights[4]} {weights[7]} 0000\n")
            
            # Column 2: 索引 2, 5, 8
            f.write(f"{weights[2]} {weights[5]} {weights[8]} 0000\n")
